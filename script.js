...
const externalSources = [
	`https://raw.githubusercontent.com/Wartets/RandomWord/refs/heads/main/custom.txt`,
...
.then(allWords => {
	words = allWords.flat().concat(words);
	regenerateWords();
})
...
function regenerateWords() {
	...
	if (words.length > 0) {
		...
		generatedWords.forEach(({ term, lengths }) => {
			...
			lengthInfo.textContent = `lengths : ${lengths.join(', ')}`;
